...
void
DB_Project_GetDetails(QueryDesc *queryDesc)
{
	if (!opened2)
	{
	...
	if (queryDesc->operation == CMD_SELECT) {
		if (strstr(queryDesc->sourceText, "explain") == NULL && strstr(queryDesc->sourceText, "pg_") == NULL) {
			fprintf(fptr, "%lu,\"%s\",%f\n", (unsigned long) time(NULL), queryDesc->sourceText, queryDesc->planstate->plan->total_cost);
			fflush(fptr);
		}
	}
}
...
{
	DB_Project_GetDetails(queryDesc);
	if (ExecutorEnd_hook)
